...
(
while true; do
sudo ip tuntap add dev tun3 mode tun; echo Creating interface tun3
sudo ip link set tun3 up; echo tun3 up
sudo ip address add 172.16.1.2/32 peer 172.16.1.1/32 dev tun3 metric 10;echo Assing ip to interfaces
for i in "${arr[@]}";do sudo ip route add $i via $gateway dev $hostinterface;done
sudo ip route add $dns via $gateway;echo Add static routes for dns and your ssh server
...
ssh \
  ...
  'interface=$(ip route | grep "default" | cut -d " " -f 5| head -n 1);echo Nat interface: $interface;
   sudo ip tuntap add dev tun3 mode tun;echo creating interface tun3 server;
   sudo ip link set tun3 up;echo tun3 up;
   ...
   sudo ip address add 172.16.1.1/32 peer 172.16.1.2/32 dev tun3 metric 10; echo tun3 ready'
